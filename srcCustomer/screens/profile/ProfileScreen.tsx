...
const THEMES = [
  { id: 'system', label: 'System Default', icon: SVG_ICONS.suitecase }, // Replace with theme icons if available
  { id: 'light', label: 'Light Mode', icon: SVG_ICONS.suitecase },
  { id: 'dark', label: 'Dark Mode', icon: SVG_ICONS.suitecase },
];

const ProfileScreen = ({ navigation }: any) => {
  const { t } = useTranslation();
  const { showToast } = useToast();
  const { colors, isDark, themeMode, setThemeMode } = useTheme(); // Use Theme Context
      ...
      >
        ...
        <View style={styles.headerCard}>
          ...
          <View style={styles.headerInfo}>
            <Text style={styles.businessName}>The Golden Fork</Text>
            <View style={styles.verifiedBadge}>
              <Text style={styles.verifiedText}>{t('profile.verified')}</Text>
            </View>
            <View style={styles.idRow}>
              <Icon
                xml={SVG_ICONS.suitecase}
                size={13}
                color={colors.textMuted}
              />
              <Text style={styles.businessId}>ID: #B2B-88392</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.statsGrid}>
          <StatCard
            ...
            onPress={() => NavigationService.navigate('TransactionScreen')}
            showHistory
            t={t}
            styles={styles}
            colors={colors}
        ...
        <View style={styles.menuList}>
          ...
          <View style={styles.dropdownContainer}>
            ...
            >
              ...
              <Text style={styles.menuLabel}>App Theme</Text>
              <Text style={styles.activeLangText}>
                {themeMode.charAt(0).toUpperCase() + themeMode.slice(1)}
              </Text>
              <View
                style={{
                  transform: [{ rotate: isThemeOpen ? '90deg' : '0deg' }],
                }}
              >
                ...
                {THEMES.map(theme => (
                  ...
                  >
                    <Text
                      style={[
                        styles.langText,
                        themeMode === theme.id && styles.activeLangLabel,
                      ]}
                    >
                      {theme.label}
                    </Text>
          ...
          <View style={styles.dropdownContainer}>
            ...
            >
              <View style={styles.menuIconContainer}>
                <Icon
                  xml={SVG_ICONS.languageIcon || SVG_ICONS.forknife}
                  color={colors.text}
                />
              </View>
              ...
              <View
                style={{
                  transform: [{ rotate: isLangOpen ? '90deg' : '0deg' }],
                }}
              >
                <Icon xml={SVG_ICONS.rightArrow} color={colors.textMuted} />
              </View>
          ...
          {/* <MenuItem
            icon={SVG_ICONS.orderBox}
            label={t('profile.live_orders')}
            onPress={() => NavigationService.navigate('OrderTrackingScreen')}
            showBadge
            styles={styles}
            colors={colors}
          /> */}
          <MenuItem
            icon={SVG_ICONS.branchesBuilding}
            label={t('profile.branches')}
            onPress={() => NavigationService.navigate('BranchLocationScreen')}
            styles={styles}
            colors={colors}
          />
          <MenuItem
            icon={SVG_ICONS.transactionHistory}
          ...
          />
          <MenuItem
            icon={SVG_ICONS.invoiceDollar}
            label={t('profile.invoices')}
            onPress={() => NavigationService.navigate('InvoiceScreen')}
            styles={styles}
            colors={colors}
          />
          <MenuItem
            ...
            onPress={() =>
              NavigationService.navigate('LegalDocScreen', {
...
const makeStyles = (colors: any, isDark: boolean) =>
  StyleSheet.create({
    ...
    headerInfo: { flex: 1 },
    businessName: { color: colors.text, fontSize: 24, fontWeight: 'bold' },
    verifiedBadge: {
    ...
    idRow: { flexDirection: 'row', alignItems: 'center', marginTop: 10 },
    businessId: { color: colors.textMuted, fontSize: 13, marginLeft: 6 },
    statsGrid: {
      flexDirection: 'row',
      flexWrap: 'wrap',
      gap: 12,
    ...
    statSub: { color: colors.textMuted, fontSize: 11, marginTop: 8 },
    progressBarBg: {
      height: 4,
